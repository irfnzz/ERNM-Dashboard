...
const Table = React.forwardRef((props, ref) => {
  ...
  const filteredItems = data.filter(item => (item.date + item.user + item.summary + item.object) && (item.date + item.user + item.summary + item.object).toLowerCase().includes(filterText.toLowerCase()));
